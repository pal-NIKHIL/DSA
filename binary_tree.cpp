...
void sumreplacement(tree* &root){        
   ...
   if(root->left_child!=NULL) root->data+=root->left_child->data;
   if(root->right_child!=NULL) root->data+=root->right_child->data;
}

bool isbalanced(tree* root){                          // time complexity=0(n)^2
    if(root==NULL) return true;
    if(isbalanced(root->left_child)==false) return false;
    if(isbalanced(root->right_child)==false) return false;
    int lh=height(root->left_child);
    int rh=height(root->right_child);
    if(abs(lh-rh)>1) return false;
    else return true;
}
bool isbalanced(tree *root,int *height){              //time complexity=0(n)
    if(root==NULL) return true;
    int lh=0,rh=0;
    if(isbalanced(root->left_child,&lh)==false) return false;
    if(isbalanced(root->right_child,&rh)==false) return false;
    *height=max(lh,rh)+1;
     if(abs(lh-rh)>1) return false;
     else return true;
...
for (int i = 0; i <number_of_node_at_kth_level; i++)
{
    ...
    if(curr->left_child!=NULL) q.push(curr->left_child);
    if(curr->right_child!=NULL) q.push(curr->right_child); 
}
}
}
